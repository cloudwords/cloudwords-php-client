...
class CloudwordsClient implements CloudwordsAPI {
  ...
  public function get_open_projects() {
    ...
    foreach( $projects_metadata as $project_metadata ) {
      $open_projects[] = new CloudwordsProject($project_metadata);
    }
  ...
  public function get_closed_projects() {
    ...
    foreach( $projects_metadata as $project_metadata ) {
      $closed_projects[] = new CloudwordsProject($project_metadata);
    }
  ...
  public function get_project($project_id) {
    ...
    return new CloudwordsProject($project_metadata);
  }
  ...
  public function approve_translated_file($project_id, $language) {
    ...
    return new CloudwordsLanguageFile($file_metadata);
  }
  ...
  public function request_bids_for_project($project_id, $preferred_vendors, $do_let_cloudwords_choose, $do_auto_select_bid_from_vendor) {
    $params = array('preferredVendors' => $preferred_vendors,
                    ...
                    'doAutoSelectBidFromVendor' => $do_auto_select_bid_from_vendor);
  ...
  public function get_bids($project_id) {
    $bids = array();
    $bids_metadata = $this->get($this->base_url_with_version . '/project/' . $project_id . '/bid.json', self::CONTENT_TYPE_JSON, self::CONTENT_TYPE_JSON);
    foreach( $bids_metadata as $bid_metadata ) {
      $bids[] = new CloudwordsBid($bid_metadata);
    }
    return $bids;
  }

  public function get_bid($project_id, $bid_id) {
    $bid_metadata = $this->get($this->base_url_with_version . '/project/' . $project_id . '/bid/' . $bid_id . '.json', self::CONTENT_TYPE_JSON, self::CONTENT_TYPE_JSON);
    return new CloudwordsBid($bid_metadata);
  }

  public function select_winning_bid($project_id, $bid_id) {
    $params = array('winningBidId' => $bid_id);
    $bid_request = $this->put($this->base_url_with_version . '/project/' . $project_id . '/bid-request', $params, self::CONTENT_TYPE_JSON, self::CONTENT_TYPE_JSON);
    return new CloudwordsBidRequest($bid_request);
  }
